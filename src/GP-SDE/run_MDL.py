...
if __name__ == '__main__':
    ...
    strategy = GeneticProgramming(fitness_function=fitness_function, num_generations=num_generations, population_size=population_size, operator_list=operator_list, variable_list=variable_list, 
                                num_populations = num_populations, layer_sizes=layer_sizes, complexity_objective=True, constant_optimization_method="gradient", constant_optimization_steps=5, 
    ...
    # Create a held-out test set (kept separate from training used during evolution)
    ...
    for seed in range(5):
        ...
        ts, ys = generate_data(data_key, env, dt, T, batch_size, obs_sigma=0.01)

        # Flatten training data grid for evaluation / MDL computation
        train_grid = ys.reshape(ys.shape[0] * ys.shape[1], ys.shape[2])
        train_drift = jax.vmap(lambda x: env.drift(0, x, jnp.array([0])))(train_grid)
        train_diffusion = jax.vmap(lambda x: env.diffusion(0, x, jnp.array([0])))(train_grid)
        ...
        for target_dim in [2]:
            ...
            _train_drift = train_drift[:, target_dim]
            _test_drift = test_drift[:, target_dim]

            _train_diffusion = train_diffusion[:, target_dim, target_dim]
            _test_diffusion = test_diffusion[:, target_dim, target_dim]

            # Get pareto front solutions
            pareto_front = strategy.pareto_front[1]

            # Compute complexity for each pareto solution
            complexities = jax.vmap(lambda s: jnp.sum(s[:,:,0] != 0))(pareto_front)

            # Compute training MSEs for drift+diffusion for each pareto solution
            drift_mses, diffusion_mses = jax.vmap(validate, in_axes=[0,None,None,None,None])(
                pareto_front, train_grid, _train_drift, _train_diffusion, strategy.tree_evaluator
            )

            # Define MDL as complexity + log(training_mse) (small regularizer added)
            eps = 1e-12
            train_mses = drift_mses + diffusion_mses
            mdl_scores = complexities + jnp.log(train_mses + eps)

            # Select solution with minimum MDL
            best_idx = jnp.argmin(mdl_scores)
            best_solution = pareto_front[best_idx]

            # Evaluate selected solution on test data
    ...
    filename = f"data/GP_SDE/{save_path}.csv"
